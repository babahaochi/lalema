...
public interface PoopRecordMapper extends BaseMapper<PoopRecord> {
    @Select("SELECT date, COUNT(*) as count FROM poop_records WHERE user_id = #{userId} AND date LIKE CONCAT(#{monthPrefix}, '%') AND deleted = 0 GROUP BY date")
    List<Map<String, Object>> getMonthRecordDays(@Param("userId") Long userId, @Param("monthPrefix") String monthPrefix);
